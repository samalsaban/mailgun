...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["tempfile"]["tmp_name"], $target_file)) {
		...
		// Write to db
		   ...
		   $fileId = rand(10,99).substr(time(),4,9).rand(1,9);
		...
		$batchcount = 0;
		$wcount = 0;
		$query_prefix = "insert into c1 (fileid,email,mail_sent) values ";
		$query_ins_append = "";
		$query_ins = "";
		for ($i = 1; $i <= $rcount; $i++) {
			$email = $emails[$i];
			$batchcount ++;
			$query_ins_append .= "($fileId,'".$email."',0),";
			$wcount ++;
