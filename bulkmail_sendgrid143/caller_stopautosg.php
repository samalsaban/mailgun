...
if ($row)	{
	if ($row['script_status'] == "COMPLETE") {
	    ...
	    // Update record in runscript_sendgrid
        $query = "update runscript_sendgrid set runsignal = '0' where script_unique_id = $script_unique_id";
	    $result = mysqli_query($dbc,$query) 
		          or die ("Error while inserting/updating record in runscript_sendgrid"); 
