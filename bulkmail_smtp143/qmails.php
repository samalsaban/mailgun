...
$usr = $_POST["usr"];
$passwd = $_POST["passwd"];
$tls = $_POST["tls"]; // values are 'Y' or 'N'
...
// Find/initialize normal_smtps_id
...
// end..Find/initialize normal_smtps_id
...
if (trim($receipent) != "") {
  ...
  $query = "insert into normal_smtpc (token,mszc) ".
           "values (".$token_t.",'".$mszc."')";
  $result = mysqli_query($dbc,$query) or die ("Error while inserting record into normal_smtpc for test");   
  // Create one record for each test email
  $lmode = "t";
  foreach ($mail_list as $testMail_val) {
	 $msgq_list_id++;
	 
     $query = "insert into normal_smtps (id,token,sub,f_mail,f_name,receipent,mail_format,server,port,usr,passwd,tls,mode) ".
              "values (".$msgq_list_id.",".$token_t.",'".$sub."','"
			  .$f_mail."','".$f_name."','".$testMail_val."','"
			  .$lmail_format."','".$server."','".$port."','".$usr."','".$passwd."','".$tls."','".$lmode."')";
     $result = mysqli_query($dbc,$query) or die ("Error while inserting record into normal_smtps for test"); 	  
  }  
  $email_count = $listsize;  
  // Send mails using process in background
  $olist = array();
  exec("php /var/www/html/bulkmail_smtp143/sendmail_t.php $token_t >> test_err &",$olist);
}
...
// Create normal_smtpc record if not created already  
// Count of normal_smtpc record
$query_chk     = "select IFNULL(count(1),0) from normal_smtpc 
			      where token = $token";
...
if ($cCount == 0) {
// Create normal_smtpc record	
  $query_content = "insert into normal_smtpc (token,mszc) ".
           "values (".$token.",'".$mszc."')";
  $result_content = mysqli_query($dbc,$query_content) or die ("Error while inserting record into normal_smtpc for bulk");  
}
// Create one record for each record fetched from triunique table for current limit
$mcount = 0; 
// Initialize query statements
$query_list = "insert into normal_smtps (id,token,sub,f_mail,f_name,receipent,mail_format,server,port,usr,passwd,tls,mode,pending_token) values ";
...
while ($row = mysqli_fetch_array($result)) {
  ...
  $query_list .= "(".$msgq_list_id.",".$token.",'".$sub."','".$f_mail."','".$f_name.
				         "','".$email_to."','".$lmail_format."','".$server."','".$port."','".$usr."','".$passwd."','".$tls."','".$lmode."',".$id."),";
  $query_upd .= "$id,";	 
...
$result_list = mysqli_query($dbc,$query_list) or die ("Error while inserting record into normal_smtps for bulk"); 
// Execute update
...
//Call Splitter
$olist = array();
exec("php /var/www/html/bulkmail_smtp143/splitter.php $token $msgq_list_start_id $msgq_list_end_id >> split_err &",$olist);
