...
//$mail_holdl  = $rowContent1['mail_holdl'];
  ...
  // Create record for runscript_mailgun if script_status='READY' else update
  if ($script_status == 'READY') {
     $query = "insert into runscript_mailgun (runid,runsignal) values (".$runid.",'1')"; 
  }
  else {
	 $query = "update runscript_mailgun set runsignal = '1' where runid = $runid";
  }
  $result = mysqli_query($dbc,$query) or die ("Error while inserting/updating record in runscript_mailgun"); 
  
  function get_runsignal (int $runid){
	 global $dbc;
     $query_signal = "select runsignal from runscript_mailgun where runid = $runid";
	 $result_signal = mysqli_query($dbc,$query_signal) or die ("Error while retrieving records from runscript_mailgun");
